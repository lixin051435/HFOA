...
const defaultURL = root + "/print/searchGzInfo";
const searchURL = root + "/print/searchGzInfo";
...
Page({
  ...
  search: function () {
    ...
    if (searchInfo.applyMan == "") {
      data.applyusername = "null";
    } else {
      data.applyusername = searchInfo.applyMan;
    }
  ...
  toDetail: function (e) {
    ...
    util.goTo("/pages/operatePages/myCheck/checkItem/sealItem/sealItem?item=" + JSON.stringify(this.data.list[id]) + "&showButton=false&needTaskId=0&showRejucted=false");
  },
  ...
  onLoad: function (options) {
    ...
    let data = {
      ...
      department: '全部',
      gzId: 0,
      applyusername: wx.getStorageSync("username")
    };
    ...
    // 获取今年该用户总览信息
    wx.request({
      url: app.globalData.URL + '/print/userPrintingCase',
      data: {
        openId: wx.getStorageSync("openId")
      },
      method: 'GET',
      ...
      success: res => {
        that.setData({
          ...
          countSecret: res.data.countSecret
        })
      },
      fail: () => {

...
// 进入页面默认的URL
const defaultURL = root + "/entertain/searchEntertainApprove";
// 搜索的URL
const searchURL = root + "/entertain/searchEntertainApprove"
...
Page({
  ...
  search: function () {
    ...
    if (searchInfo.applyMan != null && searchInfo.applyMan != "") {
      data.manager = searchInfo.applyMan;
    }else{
      data.manager = "1";
    }
  ...
  toDetail: function (e) {
    ...
    util.goTo("/pages/operatePages/myCheck/checkItem/entertainItem/entertainItem?item=" + JSON.stringify(this.data.list[id]));
  },
  ...
  onLoad: function (options) {
    ...
    // 获取今年该用户总览信息,显示在上面
    wx.request({
      url: app.globalData.URL + '/entertain/countEntertain',
      data: {
        openId: wx.getStorageSync("openId")
      },
      method: 'GET',

...
const defaultURL = root + "/applyExpenses/searchApplyExpense";
const searchURL = root + "/applyExpenses/searchApplyExpense";
...
Page({
  ...
  search: function () {
    ...
    let data = {
      ...
      searchBeginTime: searchInfo.beginTime,
      searchEndTime: searchInfo.endTime
    };
  ...
  toDetail: function (e) {
    ...
    util.goTo("/pages/operatePages/myCheck/checkItem/travelItem/travelItem?item=" + JSON.stringify(this.data.list[id]));
  },
  ...
  onLoad: function (options) {
    ...
    // 获取今年该用户总览信息
    wx.request({
      url: app.globalData.URL + '/applyExpenses/countApplyExpense',
      data: {
        openId: wx.getStorageSync("openId")
      },
      method: 'GET',
      ...
      success: res => {
        that.setData({
          useTimes: res.data.data[0].toatl || 0,

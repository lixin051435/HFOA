...
<script type="text/javascript">
	...
	*/
	...
	$(function() {
		...
		//汉化 datagrid 翻页
		$("#llbinfo").datagrid({
			url : '${pageContext.request.contextPath}/privateCar/invoiceDisplay',
			method : 'post',
			pageSize : 20,
			rownumbers : true,
			singleSelect : true,
		...
		$("#btn_ok").click(
			function() {
				...
				var pstvoucher = year+month+num;
				if(voucher.length==10){
					var param = {applyid:row.applyId,approveman:'${sessionScope.username}',vouchernum:pstvoucher,paidtime:paidTime};
					$.post(
							"${pageContext.request.contextPath}/updateInvoice",
	...
	function exportExcel(){
			...
			window.open('${pageContext.request.contextPath}/PrivateCarInvoice/exportExcel?number='+total);
	...
	function operation(value,row,rowIndex){
		//console.info(row.id);
		return '<a class="preScan" href="#" iconCls="icon-search" onclick="checkAllRows('+ row.id +','+rowIndex+')">查看明细</a>';
	}
	...
	//查看某一报销单下所有记录
	function checkAllRows(id,index){
		var now = new Date();
		...
		$('#dlg1').dialog('open').dialog('setTitle', '审核单号'+row.applyId);
		...
		$("#studentInfo").datagrid({
			url : '${pageContext.request.contextPath}/privateCar/selectByApplyIdsChild?applyid='+row.applyId,
			method : 'post',
			rownumbers : true,
			singleSelect : true,});
	}
	...
	function checkBh(id,index){
		...
		$.post(
				"${pageContext.request.contextPath}/updatePrivateCarIfUnPass",
				{parentid:row1.applyId,applyid:row2.applyId}, function(result) {
	...
	function check(id,index) {
        ...
        clock = year+"-"+cmonth+"-"+cday;
		var row = $('#llbinfo').datagrid('getData').rows[index];
		$('#dlg').dialog('open').dialog('setTitle', '审核单号'+row.applyId);
		
		$.ajax({
        	url:'${pageContext.request.contextPath}/PrivateCarInvoice/getOneRegister',
        	data:{applyid:row.applyId},
        	async:false,
        	dataType:"json",
        	type:"post",
        	success:function(result){
        			if(result.paidTime!=null&&result.voucherNum!=null){
        				var str = result.voucherNum;
            			var s1 = str.substring(0, 4);
            			var s2 = str.substring(4, 6);
            			var s3 = str.substring(6,10);
            			$("#upaidtime").textbox("setText", result.paidTime);
		...
		$("#studentInfo").datagrid({
			url : '${pageContext.request.contextPath}/selectByApplyIds?applyid='+row.applyId,
			method : 'post',
			rownumbers : true,
			singleSelect : true,});
	}
...
<body>	<!-- 审核登记对话框 -->
	<div class="mdiv" style="width:100%;height:94%;">
		<table id="llbinfo" class="easyui-datagrid" title="已报销记录"
			...
			<thead>
				<tr>
					...
					<th
						data-options="field:'applyTime',width:fixWidth(0.15),align:'center'">提交时间</th>
					<th 
						data-options="field:'applyMan',width:fixWidth(0.12),align:'center'">申请人</th>
					<th 
						data-options="field:'sum',width:fixWidth(0.1),align:'center'">金额</th>
					<th
						data-options="field:'status',width:fixWidth(0.12),align:'center'">状态</th>
					<th 
						data-options="field:'paidTime',width:fixWidth(0.12),align:'center'">审批时间</th> 
					<th
						data-options="field:'approveMan',width:fixWidth(0.12),align:'center'">审批人</th>
					<th
						data-options="field:'voucherNum',width:fixWidth(0.1),align:'center'">凭单号</th>
					<th
						data-options="field:'operation',width:fixWidth(0.15),formatter:operation,align:'center'">查看明细</th>
				</tr>
			</thead>
		</table>  
	</div>
<!-- 查询信息对话框 -->
	...
	<div id="dlg1" class="easyui-dialog"
		style="width: 60%; height: 420px; padding: 0px 0px" closed="true"
		buttons="#dlg-buttons">
			...
			<thead>
				<tr>
					<th 
						data-options="field:'userCarTime',width:fixWidth(0.08),align:'center'">用车时间</th>
					<th 
						data-options="field:'beforeDate',width:fixWidth(0.08),align:'center'">实际时间</th>
					<th 
						data-options="field:'reason',width:fixWidth(0.14),align:'center'">事由</th>
					<th 
						data-options="field:'beginAddress',width:fixWidth(0.08),align:'center'">出发地</th>
					<th 
						data-options="field:'destination',width:fixWidth(0.08),align:'center'">目的地</th>
					<th 
						data-options="field:'singleLength',width:fixWidth(0.04),align:'center'">单里里程</th>
					<th 
						data-options="field:'countLength',width:fixWidth(0.04),align:'center'">计价里程</th>
					<th 
						data-options="field:'sureLength',width:fixWidth(0.04),align:'center'">核定价格</th>
				</tr>
			</thead>
		</table>
	</div>

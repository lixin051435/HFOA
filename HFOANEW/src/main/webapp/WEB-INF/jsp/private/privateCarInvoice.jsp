...
<script type="text/javascript">
	...
	*/
	...
	$(function() {
		...
		//汉化 datagrid 翻页
		$("#llbinfo").datagrid({
			url : '${pageContext.request.contextPath}/privateCar/unSignInvoiceDisplay',
			method : 'post',
			pageSize : 20,
			rownumbers : true,
			singleSelect : true,
...
// 				 $('.preScan').linkbutton({text:'查看明细',plain:true,iconCls:'icon-search'}); 
		...
		$("#btn_ok").click(
			function() {
				...
				if(voucher.length==10){
					var param = {applyId:row.applyId,voucherNum:pstvoucher,paidTime:paidTime};
					$.post(
							"${pageContext.request.contextPath}/privateCar/updateInvoice",
	...
	function exportExcel(){
			...
			window.open('${pageContext.request.contextPath}/privateCar/privateCarInvoiceexportExcel?number='+total);
	...
	function check(id,index) {
		...
		$.ajax({
        	url:'${pageContext.request.contextPath}/privateCar/getOneRegister',
		...
		<table id="llbinfo" class="easyui-datagrid" title="已报销记录"
			...
			<thead>
				<tr>
					...
					<th 
						data-options="field:'applyMan',width:fixWidth(0.11),align:'center'">申请人</th>
					<th 
					...
					<th 
						data-options="field:'paidTime',width:fixWidth(0.1),align:'center'">审批时间</th> 
					<th
						data-options="field:'approveMan',width:fixWidth(0.11),align:'center'">审批人</th>
					<th
						data-options="field:'voucherNum',width:fixWidth(0.15),align:'center'">凭单号</th>
					<th
						data-options="field:'applyId',width:fixWidth(0.15),formatter:checkIn,align:'center'">操作</th>
				</tr>
			</thead>
		</table>  
	</div>

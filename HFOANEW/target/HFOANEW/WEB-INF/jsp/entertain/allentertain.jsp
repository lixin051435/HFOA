...
/* 	function approve(){
	...
	$(function() {
		...
		//汉化 datagrid 翻页
		$("#llbinfo").datagrid({
			url : '${pageContext.request.contextPath}/entertain/wRGetRegisterEntertain',
			method : 'post',
			pageSize : 20,
			rownumbers : true,
			singleSelect : true,
			...
			var param="number="+number;
				$.post(
						...
						param, function(result) {
									...
									$("#llbinfo").datagrid({
										url : '${pageContext.request.contextPath}/entertain/wRGetAllApproved',
										method : 'post',
										pageSize : 20,
										rownumbers : true,
										singleSelect : true,
		...
		$("#btn_cancel").click(function() {
			var row = $('#llbinfo').datagrid('getSelected');
			var uid = row.number;
			var param = "applyId="+ uid;
			$.post(
					"${pageContext.request.contextPath}/register/notThrough",
					param, function(result) {
								$("#dlg").dialog("close");
								console.info(result);
								alert("事后登记未通过审核！");
	...
	function detail(id,index) {
		...
		var playOfpath='${pageContext.request.contextPath}/view/entertain/detail.jsp?number='+row.number+'&flag=1';//跳转到明细页面
...
<body>
	<div class="mdiv" style="width:100%;height:94%;">
		<table id="llbinfo" class="easyui-datagrid" title="已驳回招待列表"
			...
			<thead>
				<tr>
					...
					<th 
						data-options="field:'entertainNum',width:fixWidth(0.06),align:'center'">招待人数</th>
					<th 
						data-options="field:'accompanyNum',width:fixWidth(0.06),align:'center'">陪同人数</th>
					<th
						data-options="field:'masterBudget',width:fixWidth(0.06),align:'center'">总预算</th>
					<th
